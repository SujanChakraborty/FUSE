...
# Patch torch.load (OGB issue with weights_only kwarg)
...
def load_dataset(dataset_name, root="."):
    ...
    # Build pyg_data so VGAE/DGI can run
...
def vgae_embedding(data, k=150, epochs=200, device='cpu'):
...
def perform_labeled_random_walks(G, label_mask, labels, num_walks=10, walk_length=5, walk_length_labelled=3):
    ...
    nodes = list(G.nodes())
    for node in tqdm(nodes, desc="Random walks per node"):
...
def compute_attention_weights(S, labeled_nodes):
    weights = {}
    for node, labeled in tqdm(labeled_nodes.items(), desc="Computing attention weights"):
        if labeled:
...
def fuse_embedding_end_orthonormalization(
    G, labels, label_mask, k=DEFAULT_EMB_DIM, eta=0.01,
    lambda_supervised=1.0, lambda_semi=2.0,
    iterations=200, initialization='random',
    num_walks=10, walk_length=5, walk_length_labelled=3, seed=None
):
    """
    FUSE embedding with orthonormalization ONLY at the end.
    ...
    # Initialize embedding matrix S
    S = np.random.randn(n, k)
    S, _ = np.linalg.qr(S)  # initial orthonormalization

    # Precompute attention weights
    labeled_walks = perform_labeled_random_walks(
        G, label_mask, labels,
        num_walks, walk_length, walk_length_labelled
    )
    attention_weights = compute_attention_weights(S, labeled_walks)

    # Gradient ascent without inner-loop QR
    for _ in range(iterations):
        ...
        # semi-supervised gradient
        grad_semi = np.zeros_like(S)
        for i in range(n):
            if (not label_mask[i]) and (i in attention_weights):
                weighted = sum(w * S[j] for j, w in attention_weights[i].items())
                grad_semi[i] = S[i] - weighted

        # update embedding
        grad = grad_modularity - lambda_supervised * grad_supervised - lambda_semi * grad_semi
        S += eta * grad
        S = S / (np.linalg.norm(S, axis=1, keepdims=True) + 1e-8)

    # FINAL ORTHONORMALIZATION (only once)
    S, _ = np.linalg.qr(S)
...
def evaluate_preds(true_int_labels, pred_int_labels):
    ...
    kappa = cohen_kappa_score(true_int_labels, pred_int_labels)
...
def run_one_experiment(dataset_name, seed, mask_frac, emb_dim=DEFAULT_EMB_DIM,
    ...
    for emb_name in embedding_methods:
        ...
        if emb_name.lower() == 'random':
            ...
            E = vgae_embedding(pyg, k=emb_dim, epochs=vgae_epochs)
            ...
            E = fuse_embedding_end_orthonormalization(G, labels_int, label_mask, k=emb_dim, iterations=fuse_iterations, seed=seed)
        else:
...
def run_benchmark(datasets, seeds, mask_fracs=[0.7, 0.3], emb_dim=DEFAULT_EMB_DIM,
    ...
    df_results.to_csv(os.path.join(save_dir, "per_run_results_all_arxiv.csv"), index=False)
    avg_by_model.to_csv(os.path.join(save_dir, "avg_by_model_and_classifier_arxiv.csv"), index=False)
    avg_embtime.to_csv(os.path.join(save_dir, "avg_embedding_times_arxiv.csv"), index=False)
