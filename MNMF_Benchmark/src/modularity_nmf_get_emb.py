...
class MNMF:
    ...
    def __init__(self, args):
        """
        Method to parse the graph, set up the similarity matrices,
        ...
        with self.computation_graph.as_default():
            ...
            # Map actual graph node IDs to indices and vice versa
            self.node_id_map = {node: i for i, node in enumerate(self.G.nodes())}
            self.reverse_node_id_map = {i: node for i, node in enumerate(self.G.nodes())}
            self.number_of_nodes = len(self.node_id_map)
            ...
            self.M = tf.Variable(tf.random_uniform([self.number_of_nodes, self.args.dimensions],
                                                   0, 1, dtype=tf.float64))
            self.U = tf.Variable(tf.random_uniform([self.number_of_nodes, self.args.dimensions],
                                                   0, 1, dtype=tf.float64))
            self.H = tf.Variable(tf.random_uniform([self.number_of_nodes, self.args.clusters],
                                                   0, 1, dtype=tf.float64))
            self.C = tf.Variable(tf.random_uniform([self.args.clusters, self.args.dimensions],
                                                   0, 1, dtype=tf.float64))
    ...
    def build_graph(self):
        ...
        self.U = self.U.assign(tf.nn.l2_normalize(tf.multiply(self.U, tf.divide(self.enum_2, self.denom_4)), axis=1))
    ...
    def update_state(self, H):
        ...
        indices = {self.reverse_node_id_map[i]: int(indices[i]) for i in range(len(indices))}
    ...
    def initiate_dump(self, session, feed_dict):
        ...
        if self.args.dump_matrices:
            cols = ["X_"+ str(x) for x in range(self.args.dimensions)]
    ...
    def optimize(self):
        """
        Run the optimization and halt when overfitting starts.
        Returns the node embeddings as a NumPy array.
        """
        ...
        with tf.Session(graph=self.computation_graph) as session:
            ...
            # Get the optimal embeddings as a DataFrame
            self.optimal_node_representations = pd.DataFrame(session.run(self.U, feed_dict=feed_dict),
                                                             columns=[f"X_{x}" for x in range(self.args.dimensions)])
            # Return as NumPy array
            return self.optimal_node_representations.to_numpy()
